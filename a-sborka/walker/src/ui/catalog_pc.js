...
function createHtmlEl(params) {
  ...
  if (type == 'panel_group' || type == 'panel_group_block_item') {
    ...
    if (inf.setIcon) {
      ...
      for (let i = 0; i < inf.setIcon.length; i++) {
        let dp = ``;
        if (i == 2 && inf.setIcon.length == 3) {
          dp = `grid-column-start: 1; grid-column-end: 3;`;
        }

        divSet += `<div style="background: ${inf.setIcon[i].color}; ${dp}">
					${inf.setIcon[i].preview}
				</div>`;
      }

      html = `<div class="${elclass}">
  ...
  if (type == 'button_panel_show') {
    html = `<div class="button_panel" style="display: none;">
...
function leftPanel(params) {
  ...
  let blockUI = createHtmlEl({ type: 'panel', parent: document.querySelector('[nameId="containerWrapper"]') });
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (arr[i].items.length > 0) {
      elBlock = childBlock({ arr: arr[i].items, paintsId: arr[i].id, parentEl: elem });
    }
  ...
  let el_bps = createHtmlEl({ type: 'button_panel_show', parent: document.querySelector('[nameId="containerWrapper"]') });
  let el_bph = createHtmlEl({ type: 'button_panel_hide', parent: document.querySelector('[nameId="containerWrapper"]') });
...
function childBlock(params) {
  ...
  let blockUI = createHtmlEl({ type: 'panel_group_block', parent: document.querySelector('[nameId="containerWrapper"]') });
...
function hideChildBlock(event) {
  ...
  for (let i2 = 0; i2 < arrElBlock.length; i2++) {
    if (arrElBlock[i2].contains(event.target)) {
      result = true;
    }
  }
