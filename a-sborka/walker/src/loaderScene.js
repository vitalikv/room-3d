...
{ 
	...
	if(url && 1==1) 
	{
		...
		p.then(data=> { loadScene_2({json: data}); })
...
async function loadScene_2(params)
{
	...
	{ 
		if(index == 'construction'){ json[index].type = index; arr[0] = json[index]; }
		else if(index == 'windows'){ json[index].type = index; arr[1] = json[index]; }
		else if(index == 'doors'){ json[index].type = index; arr[2] = json[index]; }
		else if(index == 'furnitures'){ json[index].forEach(o => o.type = index); }
...
function loadScene_3(params)
{
	...
	if(json.images)
	{
		for(let i = 0; i < json.images.length; i++)
		{
			...
			{ 
				...
				if(/exr/i.test( type )){ type = Build.infProg.mode.lightMap + '.jpg?v=' + Build.infProg.doc.vers; }
				else { type = '_x4.jpg?v=' + Build.infProg.doc.vers; }
			}	
	...
	obj.traverse(function(child)
	{
		...
		if(child.material)
		{
			...
			{
				if(json.images) { SMAT.setMatSetting_1({obj: child}); }
			}			
		}
	});									
