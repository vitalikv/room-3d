...
let t_img_man = new THREE.TextureLoader().load(img_man);
t_img_man.repeat.set(1.5, 1.5);
t_img_man.offset.set(-0.25, -0.25);
t_img_man.wrapS = t_img_man.wrapT = THREE.ClampToEdgeWrapping;
t_img_man.encoding = THREE.sRGBEncoding;
//t_img_man.anisotropy = Build.renderer.capabilities.getMaxAnisotropy();
t_img_man.needsUpdate = true;

export function initWalkPoint(params)
{
	let arr = params.arr;
	...
	//let material = new THREE.MeshPhysicalMaterial( { color: 0xffffff, side: THREE.DoubleSide, lightMap: Build.infProg.img.lightMap_1, transparent: true, opacity: 1, depthTest: false } );
	let material = new THREE.MeshPhysicalMaterial( { color: 0xffffff, side: THREE.DoubleSide, lightMap: Build.infProg.img.lightMap_1, map: t_img_man } );	
	
	let num = 1;
	
	for ( let i = 0; i < arr.length; i++ )
	{
		let area = getArea({obj: arr[i].o});				
		if(area < 1.5) continue;
		
		let m = material.clone();
		//m.map = crTexture({text: num});
		
		let circle = new THREE.Mesh( geometry, m );
		circle.position.copy(arr[i].pos);
		circle.position.y = 1;
		//circle.renderOrder = 2;
		Build.scene.add( circle );

		
		if(i == 0 && Build.infProg.doc.flatPath == 'https://files.planoplan.com/upload/userdata/1/2/projects/2770528/poplight/')
		{
			circle.position.z -= 1.5;  
		}
		
		point[point.length] = circle;
...
function initEvent()
{
	...
	container.addEventListener( 'mousedown', mouseDownWalkPoint, false );
	container.addEventListener( 'mousemove', mouseMoveWalkPoint, false );
	container.addEventListener( 'mouseup', mouseUpWalkPoint, false );	
	
	container.addEventListener( 'touchstart', mouseDownWalkPoint, false );
	container.addEventListener( 'touchmove', mouseMoveWalkPoint, false );
	container.addEventListener( 'touchend', mouseUpWalkPoint, false );	

	let elemFr = document.querySelector('[nameId="butt_camera_First"]');				
	Build.infProg.elem.butt.camFirst = elemFr;					
	elemFr.onmousedown = function(){ goFly(); }
}



function mouseDownWalkPoint( event )
{
	click.down = true;
	click.move = false;	
	click.type = 'left';
	...
	if(click.type == 'left' && Build.camera3D.userData.camera.type == 'fly')
	{
		let ray = MSV.rayIntersect( event, [...point, Build.infProg.scene.construction], 'arr' );
		if(ray.length > 0) 
		{ 
			let o = null;
			...
			for( let i = 0; i < ray.length; i++ )
			{
				...
				{
					if(ray[i].object == point[i2]) { o = ray[i].object; break; }
				}
				
				if(o) { break; }
				else if(ray[i].object.material.opacity > 0.9) { break; }
			}				
			
			if(o)
			{
				if(o.material.opacity > 0.5) { goWalk({obj: o}); }
			}						
		}			
	}
}

function mouseMoveWalkPoint( event )
{
...
export function pointRotation()
{
	...
	{	
		if(point[i].visible) point[i].rotation.copy(Build.camera3D.rotation);
	}	
}



function mouseUpWalkPoint( event )
{
	click.down = false;
	click.move = false;	
}
...
function rayPointToCam(params) 
{
	...
	else
	{
		...
		if(o.material.opacity > 0.9) { opacity = 0; }
		else { opacity = 1; }		
	}
...
export function visibleInfPoint(params)
{
	let visPoint = params.visPoint;
	
	for ( let i = 0; i < point.length; i++ )
	{
		point[i].visible = visPoint;
		if(visPoint) point[i].rotation.copy(Build.camera3D.rotation);
	}
	
	let visCursor = params.visCursor; 
	CRM.visibleCursorP360({visible: visCursor});
	
	let visButt = params.visButt;
	let display = (visButt) ? '' : 'none';
	Build.infProg.elem.butt.camFirst.style.display = display;
}
...
function goWalk(params)
{
	...
	visibleInfPoint({visPoint: false, visCursor: false, visButt: false});
...
function movePathCam(params)
{
	...
	if(params.type == 'fly') { camera3D.fov += (camera3D.userData.camera.fov.fly - camera3D.fov) * t2; }
	if(params.type == 'first') { camera3D.fov += (camera3D.userData.camera.fov.first - camera3D.fov) * t2; }
	camera3D.updateProjectionMatrix();	
	
	
	if(p1 + 1 < length) 
	{ 
		requestAnimationFrame( function() { movePathCam({path: path, path_2: path_2, type: params.type}); } ); 
	}
	else 
	{ 
		...
		{  
			visibleInfPoint({visPoint: true, visCursor: false, visButt: false});
			camera3D.fov = camera3D.userData.camera.fov.fly;
		}
		if(camera3D.userData.camera.type == 'first') 
		{
			visibleInfPoint({visPoint: false, visCursor: true, visButt: true});
