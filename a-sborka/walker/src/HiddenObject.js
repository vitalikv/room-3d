...
export function showHideObjCeil() {
  ...
  let camera = Build.camOrbit.activeCam;
  ...
  if (camera == camera3D) {
    if (camera3D.userData.camera.type == 'first') {
      ...
      if (camera.position.y > boundG.max.y) {
        value = 0;
      } else {
        value = 0;
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    let enabled = value <= 0 ? false : true;

    arr[i].visible = enabled;
  }
...
export function wallAfterRender_3() {
  ...
  let camPos = Build.camOrbit.activeCam.getWorldDirection(new THREE.Vector3());
...
function setTransparentMat_2(cdm) {
  ...
  for (let i = 0; i < arrM.length; i++) {
    ...
    // if (arrO[i].userData.infoPoint) {
    //   let infoPoint = arrO[i].userData.infoPoint;
    //   console.log(arrO[i], arrO[i].renderOrder);
    //   if (1 == 2) {
    //     infoPoint.renderOrder = arrO[i].renderOrder;
    //     infoPoint.material.transparent = arrM[i].transparent;
    //     infoPoint.material.opacity = arrM[i].opacity;
    //   } else {
    //     infoPoint.visible = arrO[i].renderOrder > 1 ? false : true;
    //   }
    // }
  }

  if (IFP.ManageIP && obj.userData.infoPoint) obj.userData.infoPoint.setOpacity();
}
